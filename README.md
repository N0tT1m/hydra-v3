...
- **Dynamic Cluster Management**: Nodes can join/leave, with layers redistributed on demand
...
| Partial SafeTensors loading (sharded and single-file) | Working |
| Tied `lm_head` / `embed_tokens` (Qwen2.5, Llama-3.2, Gemma) | Working |
| Model load / unload / hot-swap / rebalance | Working |
| Worker health, heartbeats, dead-node request termination | Working |
| API-key auth and per-key rate limiting | Working |
| MoE expert int8/int4 quantization | Working (pure torch, no bitsandbytes needed) |
| bitsandbytes `nn.Linear` quantization | CUDA only; a no-op elsewhere, with a warning |

**Not implemented** — these routes exist and return **501**, deliberately, so a
client fails fast instead of trusting a fabricated response:
...
placeholders, not partial implementations.
...
| `/v1/completions` | POST | Legacy text completion (streaming and `echo` supported) |
| `/v1/models` | GET | List loaded models |

Both completion endpoints honour `max_tokens`, `temperature` (0 selects greedy
decoding), `top_p`, `top_k` and `stop`. Stop sequences are matched against the
accumulated text, so a sequence split across two tokens is still caught.
...
| `/api/cluster/rebalance` | POST | Recompute the layer split across healthy nodes |
| `/api/models/load` | POST | Load model across workers |
| `/api/models/unload` | POST | Release the model on every worker |
| `/api/models/hot-swap` | POST | Release the active model, then load a replacement |

Rebalance, unload and hot-swap all terminate in-flight generations first — the
per-sequence KV caches belong to the layer assignment that is about to change.
Affected clients receive a final chunk with `finish_reason` set to
`rebalanced`, `model_unloaded` or `model_swapped`.

`unload` with no `model_id` releases whatever is currently active. Hot-swap
frees the old weights before loading the new ones, so a swap never needs both
models resident at once — expect a gap in availability while the new model
loads, not a zero-downtime handover.
...
```bash
make test-all          # Go unit tests (-race) + Python unit tests
make test-go           # Go only
make test-py           # Python only

make smoke             # boot the coordinator, probe /health and /ready
make integration       # cross-language protocol tests (no GPU, no download)

make test-coverage     # Go coverage, writes coverage.html
make test-coverage-py  # Python coverage, per-file misses
```

`make integration` runs three checks against a real coordinator process with
fake ZMQ workers — a register handshake, a dead-node health transition, and a
full model lifecycle (load, rebalance, unload, hot-swap) driven through the
HTTP API. They take seconds and need nothing but `pyzmq`.

Real generation against a real checkpoint is separate, because it downloads
about 1GB and takes several minutes:

```bash
make e2e               # single-worker and two-worker generation, tiny model
```

CI runs the unit tests, the smoke check and the integration scripts on every
push. The `e2e` jobs are opt-in via workflow dispatch.

#### Python test dependencies

The unit tests need a little more than the runtime: `pytest`, `pytest-asyncio`
and `pytest-cov`, plus `safetensors`, `transformers` and `psutil`, which the
tests exercise directly (synthetic checkpoints and device probing).
