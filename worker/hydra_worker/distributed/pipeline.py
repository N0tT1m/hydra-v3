...
# Pause after a loop iteration fails. Without it, a persistent error (a closed
# socket, a model that raises on every batch) turns these loops into a busy
# spin that pins a core and floods the log.
_ERROR_BACKOFF_S = 0.05
...
class PipelineNode:
    ...
    async def _recv_loop(self):
        ...
        if self.pull_socket is None:
            # Misconfiguration: a non-FIRST node with no upstream address.
            # Looping here would call recv() on None forever at full speed
            # and burn a core, so bail out and say why.
            log.error(
                "Recv loop has no upstream socket; not receiving",
    ...
    async def _send_loop(self):
        ...
        if self.push_socket is None:
            log.error(
                "Send loop has no downstream socket; not forwarding",
                node_id=self.config.node_id,
                position=self.config.position.name,
                downstream_port=self.config.downstream_port,
            )
            return
