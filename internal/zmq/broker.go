...
const (
	...
	MsgTypeModelUnloaded MessageType = "model_unloaded"
...
// Close closes all sockets and terminates the context. Idempotent.
//
// We explicitly set LINGER=0 on each socket so that sockets with queued
// outbound messages don't block Term() forever on shutdown.
func (b *Broker) Close() {
	b.mu.Lock()
	defer b.mu.Unlock()

	for i, s := range []**zmq.Socket{&b.routerSocket, &b.pullSocket, &b.pubSocket} {
		_ = i
		if *s == nil {
			continue
		}
