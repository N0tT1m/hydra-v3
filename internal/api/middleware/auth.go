...
func RateLimit(requestsPerWindow int, window time.Duration) gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		// Set rate limit headers. strconv, not string(rune(n)) — the latter
		// encodes the count as a Unicode code point, which puts a control
		// character in the header instead of a number.
